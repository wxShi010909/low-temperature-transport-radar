...
export function useReadingDockState() {
  ...
  useEffect(() => {
    ...
    setOpenState(stored ? stored === "open" : window.matchMedia("(min-width: 1280px)").matches);
  }, []);
...
export function ReadingDock({ context, open, onOpenChange }: { context: ReadingDockContext; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <aside className="reading-dock" aria-label="固定阅读助手">
      <div className="reading-dock-header"><div><span>READING DESK</span><b>阅读助手</b><small>{context.label} · {totalFavorites} 项收藏</small></div><button type="button" onClick={() => onOpenChange(false)} aria-label="收起阅读助手">×</button></div>
